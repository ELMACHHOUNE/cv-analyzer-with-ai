...
export function UploadResume() {
  ...
  return (
    <div>
      <PageHeader eyebrow="My CVs" title="Build a better version of you." description="Keep multiple versions organized, then choose the one you want to understand next." action={<Button variant="outline" asChild><Link to="/history"><MoreHorizontal className="h-4 w-4" /> View history</Link></Button>} />
      <div className="grid gap-7 xl:grid-cols-[1.05fr_0.95fr]">
        <Card className="p-5 sm:p-7">
          <div className="mb-6">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Add a version</p>
            <h2 className="mt-2 font-display text-xl font-semibold">Upload a CV</h2>
            <p className="mt-1 text-sm leading-6 text-muted-foreground">Give this version a name so it is easy to compare later.</p>
            <div className="mt-5">
              <Label htmlFor="version-name">Version name <span className="font-normal text-muted-foreground">(optional)</span></Label>
              ...
              <p id="version-name-hint" className="mt-1.5 text-xs text-muted-foreground">Up to {MAX_RESUME_NAME_LENGTH} characters. Leave it empty to use the file name.</p>
            </div>
          </div>
        ...
        <div>
          <div className="mb-5 flex items-end justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-600 dark:text-cyan-300">Saved versions</p>
              <h2 className="mt-2 font-display text-xl font-semibold">Your CV library</h2>
            </div>
          ...
          {loading ? (
            <div className="space-y-3">{Array.from({ length: 3 }).map((_, index) => <Skeleton key={index} className="h-24 rounded-2xl" />)}</div>
          ) : resumes.length ? (
            <div className="space-y-3">
              {resumes.map((resume) => {
                ...
                return (
                  <Card key={resume.id} className="group p-4 transition hover:border-primary/25">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-foreground">{resume.name || resume.originalName || 'Untitled CV'}</p>
                        <p className="mt-1 text-xs text-muted-foreground">{formatFileSize(resume.fileSize)} · {statusLabels[resume.status] || resume.status} · {formatDate(resume.createdAt)}</p>
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          <Badge variant={resume.status === 'processed' ? 'success' : resume.status === 'failed' ? 'destructive' : 'outline'}>{statusLabels[resume.status] || resume.status}</Badge>
                          {resume.analysis && <Badge variant="default">Analysis ready</Badge>}
                        </div>
                      </div>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => setDeleteTarget(resume)} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete CV</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
