    ...
    <main className="min-h-screen bg-canvas lg:grid lg:grid-cols-[minmax(0,0.95fr)_minmax(460px,1.05fr)]">
      <section className="relative hidden flex-col overflow-hidden bg-surface-dark px-10 py-10 text-on-dark lg:flex xl:px-16">
        ...
        <div className="relative my-auto max-w-2xl py-16">
          ...
          <h1 className="mt-6 text-[56px] leading-[1.05] font-bold text-balance xl:text-[64px]">
            Your CV is more than a document.
          </h1>
          <p className="mt-6 max-w-2xl text-[18px] leading-[1.55] font-light text-on-dark-soft">
            Turn scattered experience into a confident story, then see where it
            could take you next.
          </p>
          <dl className="mt-12 grid max-w-2xl grid-cols-2 gap-8 border-t border-on-dark/15 pt-8">
            {proofPoints.map((point) => (
              <div key={point.value}>
                <dt className="text-[20px] leading-[1.3] font-bold text-on-dark">
                  {point.value}
                </dt>
                <dd className="mt-2 text-[13px] leading-[1.4] font-light text-on-dark-soft">
                  {point.label}
                </dd>
              </div>
            ))}
        ...
        <p className="relative text-[12px] font-light text-on-dark-soft">
          CVision AI — career intelligence workspace
        </p>
      </section>
      ...
      <section className="flex min-h-screen flex-col px-5 py-6 sm:px-10 lg:px-16 xl:px-24">
        ...
        <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center py-12">
          {title && (
            <h2 className="text-[32px] leading-[1.15] font-bold">{title}</h2>
          )}
          {description && (
            <p className="mt-3 text-[16px] leading-[1.55] font-light text-muted">
              {description}
            </p>
          )}
  ...
  return (
    <div className="mx-auto mt-8 w-full max-w-md border border-hairline p-6">
      ...
      <p className="mt-3 text-[14px] leading-[1.55] font-light text-muted">
        Upload a CV, understand the gaps, and walk into your next application
