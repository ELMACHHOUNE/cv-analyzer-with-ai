...
const disclaimer =
  "Scores are directional, not a hiring decision. Read them with the evidence behind them.";

/* The score reads as a headline: one number, one verdict, one bar. */
export function ScoreCard({
  score,
  title = "CV score",
  description = "A directional view of clarity, completeness, and evidence.",
  breakdown = [],
  action,
  className,
}) {
  ...
  const items = safeArray(breakdown).map((item) => ({
    label: item?.label || item?.key || "Category",
    value: getScore(item?.value ?? item?.score, 0),
  }));

  return (
    <div
      className={`flex h-full flex-col border border-hairline bg-canvas p-6 sm:p-7 ${className || ""}`}
    >
      ...
      <Progress
        ...
        className="mt-5 h-2"
        indicatorClassName={meta.barClass}
      />
        ...
        <dl className="mt-8 grid gap-px border border-hairline bg-hairline sm:grid-cols-2">
          {items.slice(0, 4).map((item) => (
            <div key={item.label} className="bg-canvas p-4">
              <dt className="label-uppercase truncate text-muted">
                {item.label}
              </dt>
              <dd className="mt-2 text-[20px] leading-[1.3] font-bold tabular-nums text-ink">
                {item.value}%
              </dd>
...
export function InlineScore({ score, to, label = "View analysis" }) {
  ...
  return (
    <Link
      to={to}
      className="group inline-flex min-h-11 items-center gap-1.5 text-[13px] font-bold tracking-[0.115em] text-primary uppercase"
    >
      {label}
      <ArrowUpRight
      ...
      <span className="sr-only">
        , score {resolved ?? "not scored"} out of 100, {meta.label}
      </span>
    </Link>
  );
}
